...
📊 Diagrama de Relaciones Funcionales (Esquema Lógico)
...
🔐 usuarios_admin
Controla el acceso autenticado al panel administrativo del museo.
...
📋 especies
Catálogo taxonómico y biológico generalizado.
...
• urls_multimedia (ARRAY/TEXT): Enlaces a imágenes y recursos de audio del ejemplar.

🏷️ ejemplares_museo
Inventario físico real de los individuos presentes en la colección física o exhibición del museo.
...
📊 estadisticas_trivia
Ingesta de datos analíticos provenientes de los juegos y dinámicas gamificadas de la sala.
...
🚶 registro_visitantes
Monitoreo y control del flujo de accesos a la sala de exhibición.
...
Historial detallado de consultas guiadas por Inteligencia Artificial conversacional dentro de la sala.
...
Para proteger tu sistema y los datos del museo, sigue estas obligaciones:

🔐 Credenciales y Variables de Entorno:
• Crea un archivo .env en la raíz del proyecto para almacenar variables sensibles como la URL de la base de datos y la SECRET_KEY.
• Asegúrate de que el archivo .env esté en tu .gitignore.
• Para generar una SECRET_KEY segura, usa un generador criptográfico (ej. en Linux/macOS: openssl rand -hex 32).

📦 Dependencias:
• Revisa siempre el archivo requirements.txt antes de ejecutar pip install. Asegúrate de que todas las librerías provengan de fuentes oficiales (PyPI) y que sus nombres sean correctos para evitar ataques de "typosquatting".

🗄️ Base de Datos:
• Usa contraseñas fuertes y únicas para el usuario de la base de datos.
• Realiza respaldos regulares de la base de datos, especialmente antes de ejecutar migraciones.

🌐 Entorno de Producción:
• Nunca ejecutes el servidor de desarrollo de Flask/Django (u otro framework) en un entorno de producción. Usa un servidor WSGI como Gunicorn o uWSGI.
• Mantén todas las librerías actualizadas para parchear vulnerabilidades conocidas.
